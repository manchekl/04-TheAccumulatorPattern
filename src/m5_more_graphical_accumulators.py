...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range (m):
        ...
        x=rectangle.get_center().x-(rectangle.get_width()/2)-radius
        center=rg.Point(x-(k*2*radius),rectangle.get_center().y)
    ...
    for k in range (n):
        ...
        y=rectangle.get_center().y-(rectangle.get_height()/2)-radius
        center=rg.Point(rectangle.get_center().x,y-(k*2*radius))
        circle=rg.Circle(center,radius)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range (n):
        xchange=k*(rectangle1.get_width()/2)
        ychange=k*(rectangle1.get_height()/2)

        start=rg.Point(rectangle1.get_center().x-xchange,rectangle1.get_center().y+ychange)
        end=rg.Point(rectangle2.get_center().x-xchange,rectangle2.get_center().y+ychange)
